...
{
    float OriginDistance = UKismetMathLibrary::Vector_Distance(GetOwningPlayer()->GetPawn()->GetActorLocation(), TargetActor->GetActorLocation());
...
void UIndicatorWidget::UpdatePosition()
{
    ...
    Test();
    SetClippingPosition();
    if(ClippingPosition != FVector2D::ZeroVector)
    {
        // ClippingPosition
...
void UIndicatorWidget::SetPrimitivePosition()
{
    ...
    FVector CameraX = UKismetMathLibrary::ProjectVectorOnToVector(CameraToTargetLocation, TargetActor->GetActorRightVector());
    FVector CameraY = UKismetMathLibrary::ProjectVectorOnToVector(CameraToTargetLocation, TargetActor->GetActorUpVector());
...
void UIndicatorWidget::SetClippingPosition()
{
    float TargetX = PrimitivePosition.X;
    float TargetY = PrimitivePosition.Y;
    float RelativeX = RelativePosition.X;
    float RelativeY = RelativePosition.Y;
    float WindowsSizeX = DefaultWindowsSize.X;
    float WindowsSizeY = DefaultWindowsSize.Y;
    ...
    if(Area == 0)
    {
        ClippingPosition = FVector2D::ZeroVector;
        return;
    }
    ...
    else if(X > WindowsSizeX || X < 0.0f || Y > WindowsSizeY || Y < 0.0f)
    {
        ClippingPosition = FVector2D::ZeroVector;
        return;
    }
    ...
    {
        ClippingPosition.X = X;
        ClippingPosition.Y = Y;
        return;
    }
