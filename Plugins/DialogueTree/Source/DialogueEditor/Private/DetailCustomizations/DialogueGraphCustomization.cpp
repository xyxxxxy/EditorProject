...
void FDialogueGraphCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	UDialogueEdGraph* TargetGraph = Cast<UDialogueEdGraph>(TargetObjects[0].Get());
	if(!TargetGraph)
	{
		return;
	}
	...
	IDetailCategoryBuilder& CategoryBuilder = DetailBuilder.EditCategory("Dialogue");
